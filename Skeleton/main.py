...
def main():
    ...
    while (cap.isOpened()):
        ...
        keypoints = cv2.goodFeaturesToTrack(grey, 200, 0.01, 10, mask=None, blockSize=3, useHarrisDetector=False,
                                            k=0.04)
        ...
        if not len(last_keypoints) == 0:
            ...
            for i in range(len(points)):
                ...
                otherP = (np.rint(otherP)).astype(int)
                flowPoints += 1

                color = tuple([0, 255, 0])
                cv2.circle(last_frame, tuple((np.rint(last_keypoints[i])).astype(int)), 1, color)
                cv2.line(last_frame, tuple((np.rint(last_keypoints[i])).astype(int)), tuple(otherP), color)
